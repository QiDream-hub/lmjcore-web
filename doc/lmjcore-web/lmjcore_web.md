...
LMJCore 使用两个 LMDB 数据库协同工作，将任意嵌套的数据结构拆解为扁平的、可高效访问的片段：

| 空间 | 名称 | 用途 | 关键特性 |
|------|------|------|----------|
| **集合区** | `set` | 存储实体的关联项集合（对象的成员名列表、集合的元素列表） | 启用 `MDB_DUPSORT`，按字典序自动排序，**不保留插入顺序** |
| **主存储区** | `main` | 存储对象成员的具体值 | Key = `[17B 实体指针][成员名]`，点查 O(1) |
...
set（集合区）：
┌────────────────┬─────────────┐
│ Key: ptr_set   │ Value: "apple"  │ ← LMDB 按字典序排序
├────────────────┼─────────────┤
│ Key: ptr_set   │ Value: "banana" │
└────────────────┴─────────────┘
```

> ⚠️ **重要**: 集合是**无序**的，不保留插入顺序，自动去重并按字典序排序。
...
> ⚠️ **空键自动删除**: 由于 `set` 基于 LMDB 的重复键（`MDB_DUPSORT`）实现，**不允许空键**。当删除实体的最后一个成员/元素时，该键会被 LMDB 自动删除，实体随之消失。

#### 4.2.1 删除最后一个成员/元素的后果
