...
int register_all_routes(router_t *router) {
  ...
  // POST /obj - 创建空对象
  if (router_register(router, HTTP_POST, "/$'obj'", handle_obj_create) != 0) {
    fprintf(stderr, "Failed to register POST /obj\n");
    return -1;
  }

  // GET /obj/{ptr} - 获取完整对象
  if (router_register(router, HTTP_GET, "/$'obj'/${}", handle_obj_get) != 0) {
  ...
  if (router_register(router, HTTP_DELETE, "/$'obj'/${}/${}",
                      handle_obj_member_del) != 0) {
    fprintf(stderr, "Failed to register DELETE /obj/{ptr}/{member}\n");
    return -1;
  }

  // GET /obj/query - 链式查询
  if (router_register(router, HTTP_GET, "/$'obj'/$'query'", handle_obj_query) !=
      0) {
  ...
  // POST /set - 创建空集合
  if (router_register(router, HTTP_POST, "/$'set'", handle_set_create) != 0) {
    fprintf(stderr, "Failed to register POST /set\n");
    return -1;
  }

  // GET /set/{ptr} - 获取完整集合
  if (router_register(router, HTTP_GET, "/$'set'/${}", handle_set_get) != 0) {
  ...
  // GET /health - 健康检查
  if (router_register(router, HTTP_GET, "/$'health'", handle_health) != 0) {
